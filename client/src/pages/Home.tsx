...
export default function Home() {
  ...
  const pdfAttachments = useMemo(() => uploadItems.filter((item) => item.kind === "pdf"), [uploadItems]);
  ...
  function handleFiles(files: FileList | File[]) {
    ...
    incoming.forEach((file) => {
      ...
      next.push({ id: `${file.name}-${file.size}-${Date.now()}-${Math.random()}`, file, kind: validation.kind!, status: "Ready" });
    });
  ...
  function removeFile(id: string) { setUploadItems((current) => current.filter((item) => item.id !== id)); }
  ...
  async function processDocuments() {
    ...
    const source = initial.find((item) => item.kind === "pdf")?.file.name || initial[0].file.name;
    setFields(createDemoFields(source));
    setWorkflowStep("cross-check"); setProcessingNote("Cross-check complete — matching values found across the demo shipment record.");
  ...
  async function downloadEml() {
    ...
    const eml = await buildEml(emailSubject, emailHtml, emailText, pdfAttachments.map((item) => ({ name: item.file.name, file: item.file })));
  ...
  return <AppShell screen={screen} onNavigate={navigate}><Topbar screen={screen} onNew={() => navigate("new")} />{showEmail ? renderEmail() : content}<footer className="mx-auto max-w-[1420px] px-5 pb-7 pt-1 text-[10px] text-[#9aabb8] md:px-9"><div className="flex flex-col justify-between gap-2 border-t border-[#dfe7ee] pt-4 sm:flex-row"><span>Split Manifest Email Generator · Demo workspace</span><span className="flex items-center gap-1.5"><ShieldCheck className="h-3 w-3 text-[#3e9e70]" /> No automatic sending</span></div></footer></AppShell>;
}
