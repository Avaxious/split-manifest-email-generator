...
type RecipientGroup = { id: string; name: string; addresses: string[] };
const FIXED_CC = ["kaivan@arameshabi.com"];
...
export default function Home() {
  ...
  const [recipientGroups, setRecipientGroups] = useState<RecipientGroup[]>(() => { try { return JSON.parse(localStorage.getItem("split-manifest-recipient-groups") || "[]"); } catch { return []; } });
  const [selectedRecipientGroup, setSelectedRecipientGroup] = useState(() => localStorage.getItem("split-manifest-selected-recipient-group") || "");
  ...
  useEffect(() => { localStorage.setItem("split-manifest-recipient-groups", JSON.stringify(recipientGroups)); }, [recipientGroups]);
  useEffect(() => { localStorage.setItem("split-manifest-selected-recipient-group", selectedRecipientGroup); }, [selectedRecipientGroup]);
  function addRecipientGroup() { const name = window.prompt("Group name"); const raw = window.prompt("Email addresses separated by commas"); if (!name || !raw) return; setRecipientGroups((groups) => [...groups, { id: `${Date.now()}`, name: name.trim(), addresses: raw.split(",").map((address) => address.trim()).filter(Boolean) }]); }
  function editRecipientGroup(group: RecipientGroup) { const name = window.prompt("Group name", group.name); const raw = window.prompt("Email addresses separated by commas", group.addresses.join(", ")); if (!name || !raw) return; setRecipientGroups((groups) => groups.map((item) => item.id === group.id ? { ...item, name: name.trim(), addresses: raw.split(",").map((address) => address.trim()).filter(Boolean) } : item)); }

  const requiredMissing = useMemo(() => getMissingRequiredFields(fields), [fields]);
  const conflictingFields = useMemo(() => FIELD_DEFINITIONS.map(({ key }) => fields[key]).filter((field) => field.status === "Conflict").map((field) => field.label), [fields]);
  ...
  function updateField(key: FieldKey, value: string) {
    setFields((current) => ({ ...current, ...(key === "container_number" ? { containers: value.split(" /").map((item) => item.trim()).filter(Boolean) } : {}), [key]: { ...current[key], value, status: value === current[key].originalValue ? current[key].status : "Manually Corrected" } }));
  }
  ...
  function generateEmail() {
    ...
    toast.success("Email draft generated", { description: "The selected TO group and fixed CC were added." });
  }
  ...
  async function downloadEml() {
    ...
    const eml = await buildEml(emailSubject, emailHtml, emailText, emailAttachments.map((item) => ({ name: item.file.name, file: item.file })), { to: selectedGroup?.addresses ?? [], cc: FIXED_CC });
